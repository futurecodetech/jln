...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #JulianTEST2\r");
            Send(hSocket, "WHO #JulianTEST2\r");
        } else {
            // randomly join #Julian00-#Julian05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Julian%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Julian%02d\r", channel_number).c_str());
        }
